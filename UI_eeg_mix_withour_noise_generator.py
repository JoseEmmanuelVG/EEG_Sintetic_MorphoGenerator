...
def pink_noise(n_samples):
    ...
    noise = noise/np.max(np.abs(noise)) * random.uniform(17.5, 20)  # Normalize and scale to 20-60 µV
    return noise
...
for i in range(n_channels):
    ...
    n_groups = 2  # Generate the same number of spike-wave groups for all the channels
...
# Calculate bipolar data and channel names
for i in range(n_channels):

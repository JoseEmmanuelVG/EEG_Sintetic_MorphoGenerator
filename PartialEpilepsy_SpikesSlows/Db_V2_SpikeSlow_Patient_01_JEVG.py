...
def generate_spike_slow_wave(sfreq, amplitude_spike_range, duration_spike_range, amplitude_slow_range, duration_slow_range):
    ...
    spike_wave = generate_spike(amplitude_spike, (duration_spike, duration_spike), sfreq)
...
event_channels_list = [
    ["Fp1", "F7", "T3", "F3", "C3", "P3", "F4", "C4", "P4", "Fp2", "F8", "T4", "T6"],
    ["Fp1", "F7", "T3", "F3", "C3", "P3", "F4", "C4", "P4", "Fp2", "F8", "T4", "T6"],
    ["Fp1", "F7", "T3", "F3", "C3", "P3", "F4", "C4", "P4", "Fp2", "F8", "T4", "T6"],
    ["Fp1", "F7", "T3", "F3", "C3", "P3", "F4", "C4", "P4", "Fp2", "F8", "T4", "T6"],
    ["Fp1", "F7", "T3", "F3", "C3", "P3", "F4", "C4", "P4", "Fp2", "F8", "T4", "T6"],
...
for case, start in enumerate(range(0, total_duration, interval_duration)):
    ...
    for channel_name in event_channels:
        ...
        for t in slow_wave_intervals:
            spike_slow_wave = generate_spike_slow_wave(sfreq=sampling_freq,
                                                       amplitude_spike_range=(30, 80),
                                                       duration_spike_range=(0.01, 0.03),
                                                       amplitude_slow_range=(40, 80),
                                                       duration_slow_range=(0.22, 0.57))
            start_idx = int(event_start + t)
            end_idx = start_idx + len(spike_slow_wave)
            eeg_signals[channel_idx, start_idx:end_idx] += spike_slow_wave
...
base_folder = Path("MonoEEG_Patient_1_Sp_Sl_JEVG")
index = 1
...
fif_filename = current_folder / "MonoEEG_Patient_1_Sp_Sl_JEVG_raw.fif"
...
txt_filename = current_folder / "MonoEEG_Patient_1_Sp_Sl_JEVG_text.txt"
...
edf_filename = current_folder / "MonoEEG_Patient_1_Sp_Sl_JEVG_EegFormat.edf"
...
for i in range(10):
    ...
    fig.savefig(current_folder / f'MonoEEG_Pat_1_SpSl_JEVG_segment_{i+1}.jpg')
    plt.close(fig)
